...
<div>
  ...
  <span className='text-ascent-2 text-sm'>{moment(reply?.createdAt?? "2024-01-08").fromNow()}</span>
</div>
</div>
<div  className='ml-12'>
...
<span className='text-blue cursor-pointer' onClick={()=>setReplyComments(comment?._id)}>Reply</span>
</div>
{
...
{comment?.replies?.length>0 && (
  ...
  onClick={()=>
    setShowReply(showReply === comment?.replies?._id
    ? 0
    : comment?.replies?._id)
  }
...
<Link to={"/proflie/"+ comment?.userId?._id}>

<p className='font-medium text-base text-ascent-1'>{comment?.userId?.firstName}{comment?.userId?.lastName}</p>

</Link>
<span  className='text-ascent-2 text-sm'>{moment(comment?.createdAt ?? "2024-01-08").fromNow()}</span>
 </div>
